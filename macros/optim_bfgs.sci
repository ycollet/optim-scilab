function [x_opt, x_history] = optim_bfgs(x0, bfgs_f, bfgs_df, h, Log, ItMX, ls_ItMX, lsm, TOL, StepTOL, XTOL, SubTOL, Restart)
//
//////////////////
// Informations //
//////////////////
...
for It=1:ItMX
  ...
  if (Log) then
    scf(); plot_d(_bfgs_f, xk_1, d, lbounds, ubounds, %T, 0, max([h abs(step(1))]));
    printf('optim_bfgs: close the graph to continue - Iteration %d / %d\n', It, ItMX);
    xclick();
    xdel();
  end
  ...
  y  = _bfgs_f(xk_1);
	if ((abs(norm(ys))<TOL)) then
	  if Log then
      printf('optim_bfgs: convergence reached:\n')
      printf('%10s %10s\n', 'f(x)', '||fs(x)||');
      printf('%10.2e %10.2e\n', y, norm(ys));
    end
    x_opt = xk_1;
	...
	elseif ((step(1)<StepTOL)|(abs(norm(xk_1-xk_2))<XTOL)) then
	  if Log then
      printf('optim_bfgs: optimization stopped:\n')
      printf('%10s %10s\n', 'f(x)', '||fs(x)||');
      printf('%10.2e %10.2e\n', y, norm(ys));
    end
    x_opt = xk_1
