function [pop_opt, fobj_pop_opt, pop_init, fobj_pop_init] = optim_nsga(ga_f, pop_size, nb_generation, p_mut, p_cross, init_func, param, Log, crossover_func, mutation_func, codage_func, selection_strategy, nb_couples, pressure, sigma, pow)
...
for i=1:size(Aux_Total_FObj,1)
  Niching(i) = 0;
  for j=1:size(Aux_Total_FObj,1)
    Distance = sqrt(sum((Aux_Total_FObj(i,:) - Aux_Total_FObj(j,:)).^2));
    if Distance < sigma then
      Niching(i) = Niching(i) + (1 - Distance / sigma)^pow;
    end
  end
end
// Apply niching penality to fobj
Total_FObj = Total_FObj ./ Niching;
...
for i=1:nb_generation
  ...
  for j=1:size(Aux_FObj_Indiv1,1)
    ...
    for k=1:size(Aux_Total_FObj,1)
      Index3 = Index3 + and(Aux_FObj_Indiv1(j,:)<=Aux_Total_FObj(k,:)) & or(Aux_FObj_Indiv1(j,:)<Aux_Total_FObj(k,:));
    end
  ...
  case 'elitist' then
    ...
    for j=1:size(Aux_Total_FObj,1)
      Niching(j) = 0;
      for k=1:size(Aux_Total_FObj,1)
        Distance = sqrt(sum((Aux_Total_FObj(j,:) - Aux_Total_FObj(k,:)).^2));
        if Distance < sigma then
    ...
    error('optim_nsga: wrong selection strategy');
  end
end
