function [optarg,optval,nfeval] = DiffEvol(fct,VTR,XVmin,XVmax,USERDATA,NP, itermax,F,CR,strategy,report);
...
if ~isdef('fct','local') then
  error('DiffEvol 1st argument must be function name'); 
end
if ~isdef('VTR','local') then
  VTR = 1.e-6;
end

if ~isdef('XVmin','local') then
  XVmin = [-2 -2]';
end

if ~isdef('XVmax','local') then
  XVmax = [2 2]';
end

D = max([size(XVmax,1) size(XVmax,2)]);

if ~isdef('USERDATA','local') then
  USERDATA = [];
end

if ~isdef('NP','local') then
  NP = 10*D;
end

if ~isdef('itermax','local') then
  itermax = 200;
end

if ~isdef('strategy','local') then
  strategy = 7
end

if ~isdef('F','local') then
  F = [0.8;0.6];
end

F= F(1);

if ~isdef('CR','local') then
  CR = 0.5
end

if ~isdef('report','local') then
  report = 10;
end

param = macrovar(fct);
TwoParams = length(param(1))==2;
...
for i=1:NP
   pop(:,i) = XVmin + rand(size(XVmin,1),size(XVmin,2)).*(XVmax - XVmin);
end;

popold   = zeros(pop);  // toggle population
val      = zeros(NP,1); // create and reset the "cost array"
optarg   = zeros(size(XVmin,1),size(XVmin,2));  // best population member ever
optargit = zeros(size(XVmin,1),size(XVmin,2));  // best population member in iteration
...
for i=2:NP                  // check the remaining members
  if TwoParams then
    val(i) = fct(pop(:,i),USERDATA);
  else
    val(i) = fct(pop(:,i));
  end
  nfeval = nfeval + 1;
  if (val(i) < optval) then // if member is better
...
while ((iter < itermax) & (optval > VTR))
  ...
  for i=1:NP
    if TwoParams then
      tempval = fct(ui(:,i),USERDATA); // check cost of competitor
    else
      tempval = fct(ui(:,i)); // check cost of competitor
    end
    nfeval  = nfeval + 1;
    if (tempval <= val(i)) then      // if competitor is better than value in "cost array"
       pop(:,i) = ui(:,i);           // replace old vector with new one (for new iteration)
       val(i)   = tempval;           // save value in "cost array"

       // we update optval only in case of success to save time
       if (tempval < optval) then // if competitor better than the best one ever
