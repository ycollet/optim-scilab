...
printf('steepestdemo: start point:\nx=[%.4e %.4e], ||f''(x)||=%4e\n', x0(1), x0(2), fs(x0));

[x_opt, x_history] = optim_steepest(x0, f, fs, h, Log, ItMX, ls_ItMX, lsm, TOL, StepTOL, XTOL, SubTOL);

printf('steepestdemo: minimum not found, last point:\nx=[%.4e %.4e], ||f''(x)||=%4e\n', x_opt(1), x_opt(2), fs(x_opt));
